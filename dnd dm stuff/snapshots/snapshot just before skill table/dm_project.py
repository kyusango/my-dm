...
def generate_character_sheet(character_data, file_name="character_sheet.pdf"):
    ...
    # Saving Throws
    y_position -= 40
    c.drawString(50, y_position, "Saving Throws:")
    for save, value in character_data.get("saving_throws", {}).items():
        y_position -= 20
        c.drawString(70, y_position, f"{save}: {value}")
    ...
    for skill, value in character_data.get("skills", {}).items():
        y_position -= 20
        c.drawString(70, y_position, f"{skill}: {value}")
...
# SECTION: Update Subclasses Function
